...
public class 小耐久1 : DongZuoBase
{
    ...
    public override void 动作()
    {
        ...
        计时器.执行(1.0f, () =>
        {
            ...
            计时器.执行(0.1f, () =>
            {
                发射弹幕(transform.position, Random.Range(0, 360), Random.Range(2f, 4f), 0, 0, Color.white);
            }, 100);
            计时器.执行(1f, () =>
            {
                // 向玩家方向发射弹幕
    ...
    protected override void 重置()
    {
        ...
        存档.SetActive(true);
        消失的单位.SetActive(true);
        出现的单位.SetActive(false);
