...
public class 小耐久2 : DongZuoBase
{
    ...
    public override void 动作()
    {
        ...
        计时器.执行(1.0f, () =>
        {
            ...
            计时器.执行(0.8f, () =>
            {
                for (int i = 0; i < 10; i++)
                {
                    发射弹幕(transform.position, 36*i, Random.Range(6f, 7f), 0, 0, Color.yellow);
                }
            }, 13);
            var a = 0;
            计时器.执行(1f, () =>
            {
                ...
                var 角度 = Mathf.Atan2(player.transform.position.y - transform.position.y, player.transform.position.x - transform.position.x) * Mathf.Rad2Deg;
                    ...
                    if (a++%2 == 0)
                    {
                        var danmu = 发射弹幕(transform.position, 角度-30, 5, 0, 0, Color.red);
                        danmu.加速度 = 0.04f;
                        danmu.角速度 = 1.5f;
                    }
                    else
    ...
    Danmu 发射弹幕(Vector3 position, float angle, float speed, float 加速度 = 0, float 角速度 = 0, Color color = new Color() ,float 自动删除时间 = 5f)
    {
        GameObject danmu;
        ...
        GameObject.Destroy(danmu1.gameObject, 自动删除时间);
        return danmu1;
    }
