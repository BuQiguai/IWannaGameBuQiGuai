    ...
    public GameObject InstancePlace(float x, float y, string tag)
    {
        ...
        foreach (var i in cders)
        {
            if (i == this) // Don't check himself
                continue;
            if(i.gameObject.activeInHierarchy == false) // Don't check inactive object
                continue;
            if (i.maskRenderer == null) // Don't check object without sprite renderer
                continue;
            if (i.maskRenderer.sprite == null) // Don't check object without sprite
                continue;
            // Update mask data if using sprite animator
